...
$zonas = []; // Variable para almacenar todas las zonas
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $result = mysqli_query($conn, "SELECT id, nombre, distrito FROM zonas");
...
<head>
    ...
    <script>
        ...
        function updateZonas() {
            ...
            zonaSelect.innerHTML = ''; // Limpiar el select de zonas

            const filteredZonas = zonas.filter(zona => zona.distrito === distrito);

            if (filteredZonas.length > 0) {
                filteredZonas.forEach(zona => {
                ...
                const option = document.createElement('option');
                option.text = 'No hay zonas disponibles';
                zonaSelect.appendChild(option);
            }
        }
    </script>
    ...
    <main class="container mt-5">
	    ...
	    <form action="" method="POST">
          ...
          <div class="col">
              ...
              <select class="form-select" id="distrito" name="distrito" onchange="updateZonas()">
